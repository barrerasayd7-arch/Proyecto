...
export const seguirUsuario = async (req, res) => {
  ...
  try {
    ...
    const existe = await conn
      ...
      .query(
        "SELECT 1 FROM seguidores WHERE id_seguidor = @id_seguidor AND id_seguido = @id_seguido",
      );
...
export const estadoSeguimiento = async (req, res) => {
  ...
  try {
    ...
    const result = await conn
      ...
      .query(
        "SELECT COUNT(*) AS total FROM seguidores WHERE id_seguidor = @id_seguidor AND id_seguido = @id_seguido",
      );
