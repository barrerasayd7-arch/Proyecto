...
function FormCalificacion({ servicioId, showModal, onNuevaResena }) {
  ...
  useEffect(() => {
    ...
    fetch(
      `${API_CALIFICACIONES}/puede-calificar?id_cliente=${id_cliente}&id_servicio=${servicioId}`,
    )
      .then((r) => r.json())
  ...
  const handleEnviar = async () => {
    if (estrellas === 0) {
      showModal("error", "❌ Selecciona una puntuación");
      return;
    }
    const id_cliente = Number(localStorage.getItem("usuarioId"));
    setEnviando(true);
      ...
      >
        {[1, 2, 3, 4, 5].map((n) => (
          <span
            ...
            style={{
              cursor: "pointer",
              color: n <= (hover || estrellas) ? "#fbbf24" : "#374151",
            }}
